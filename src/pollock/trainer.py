...
from pollock.dataloader import sanity_check_dataloader
from pollock.helpers import (
...
from pollock.models import pollockModel, build_model
from pollock.models.base import check_model_has_grad
from pollock.models.llama import LlamaForTraining, RotaryEmbedding
from pollock.models.starcoder2 import Starcoder2ForTraining
from pollock.optim.clip_grads import clip_grad_norm
from pollock.parallel import ParallelContext
from pollock.parallel.data_parallel.utils import sync_gradients_across_dp
...
from pollock.parallel.tensor_parallel.nn import TensorParallelRowLinear
...
from pollock.random import set_random_seed
from pollock.sanity_checks import (
    after_optim_step_sanity_checks,
    after_tbi_sanity_checks,
    before_optim_step_sanity_checks,
    before_tbi_sanity_checks,
)
from pollock.serialize import (
    load_lr_scheduler,
    load_meta,
    load_weights,
    parse_ckpt_path,
...
from pollock.serialize.optimizer import load_optimizer
...
class DistributedTrainer:
    def __init__(
        ...
        Args:
            ...
            model_class: The `pollockModel` class to use (for example `LlamaForTraining`). Defaults to `None` which will use the model class defined in the config.
        """
        ...
        # Log benchmark info
        if os.environ.get("pollock_BENCHMARK", "0") == "1":
        ...
        self.unwrapped_model: pollockModel = (
    ...
    def pre_training(self, *args, **kwargs):
        ...
        if dist.get_rank(self.parallel_context.world_pg) == self.logger_ranks[0] and wandb is not None:
            wandb.init(
                ...
                config={"pollock_config": self.config.as_dict()},
            )
    ...
    def train_step_logs(
        ...
        # pollock Benchmark mode: we log the throughput and exit
        if os.environ.get("pollock_BENCHMARK", "0") == "1" and self.iteration_step == 3:
            log_throughput(
                self.config,
    ...
    def init_model(self) -> Union[pollockModel, DistributedDataParallel]:
    ...
    def _init_model_instance(self) -> pollockModel:
    ...
    def _init_model(
        self,
        model_builder: Callable[[], pollockModel],
        ...
        # Sanity check the model, all parameters must be pollockParameter (either tied or sharded)
...
def mark_tied_parameters(
    model: pollockModel, parallel_context: ParallelContext, parallel_config: Optional[ParallelismArgs] = None
):
    # Tie embeddings
...
def mark_unsharded_params_as_tied_across_tp(
    model: pollockModel, parallel_context: ParallelContext, parallel_config: "ParallelismArgs"
):
